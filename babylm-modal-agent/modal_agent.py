...
def tokens_to_label(tokens: int) -> str:
    # 1_000_000 -> "1m", 50_000_000 -> "50m"
...
def run_mono_trial(
    ...
    tokens_label = tokens_to_label(max_tokens)

    run_name = f"large-{lang}-{tokens_label}-{epochs}ep-{lr_label}-{batch_size}b"
...
def main():
    RUN_BILINGUAL = False #set False when you only want monolingual
    ...
    #monolingual runs
    if RUN_MONOLINGUAL:
        # ── Train ONE language at a time ──
        # eng / nld already done (see sheet), so only ind and jav remain.
        # Run "ind" first; when it finishes, change this to "jav" and run again.
        LANG_TO_RUN = "nld"   # <-- switch to "jav" for the next batch

        TOKEN_BUDGETS = [1_000_000, 3_000_000, 10_000_000, 30_000_000]

        mono_trials = [
            # lang, tokens, lr, epochs, batch_size
            (LANG_TO_RUN, tokens, 2e-4, 8, 32)
            for tokens in TOKEN_BUDGETS
        ]

        for lang, tokens, lr, epochs, batch_size in mono_trials:
        ...
        print(f"Started {len(mono_trials)} monolingual jobs for '{LANG_TO_RUN}'.")
